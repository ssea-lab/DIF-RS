"""A meta PyTorch Lightning model for training and evaluating DIF-RS models."""
...
class COMetaModel(pl.LightningModule):
  ...
  #   all_metrics = {k: [] for k in outputs[0].keys()}
  ...
  def test_dataloader(self):
    batch_size = 1
    # self.test_dataset = torch.utils.data.Subset(self.test_dataset,range(2))
  ...
  def val_dataloader(self):
    batch_size = 1
    self.args.validation_examples = min(self.args.validation_examples , len(self.validation_dataset))
    ...
    # 获取 self.validation_dataset 的长度
    dataset_size = len(self.validation_dataset)
    # 创建随机采样器
    sampler = SubsetRandomSampler(torch.randperm(dataset_size)[:self.args.validation_examples])
    # 使用采样器来创建 DataLoader
    val_dataloader = GraphDataLoader(self.validation_dataset, sampler=sampler, batch_size=batch_size)
