"""Lightning module for training the DIF-RS TSP model."""
  ...
  def test_step(self, batch, batch_idx, split='test'):
    ...
    for _ in range(self.args.sequential_sampling):
      ...
      for i in range(steps):
        ...
        #     ax.plot(np_points[i, 0], np_points[i, 1], 'o', color='black')
        #     ax.text(np_points[i, 0], np_points[i, 1], f'{i}', fontsize=12, ha='right')
  
        #   # 绘制路径
        #   for i in range(20):
        #     for j in range(20):
        #       if i != j:
        #         reachability = xt1[0, i, j]
        #         if reachability > 0:
        #           ax.plot([np_points[i, 0], np_points[j, 0]], [np_points[i, 1], np_points[j, 1]], 
        #                   color=(0, 0, 0, reachability), linewidth=2 * reachability)
  
        #   ax.set_xlabel('X')
        #   ax.set_ylabel('Y')
        #   ax.set_title('Nodes and Paths')
        #   plt.show()
